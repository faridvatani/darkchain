...
export const CutCornerButton = (props: ComponentPropsWithoutRef<"button">) => {
  const { className, children, type = "button", disabled } = props;
  return (
    <button
      type={type}
      disabled={disabled}
      className={twMerge(
        "relative bg-fuchsia-500/20 px-4 py-2 font-extrabold uppercase font-heading text-sm tracking-wide disabled:bg-zinc-500 disabled:cursor-not-allowed [mask-image:linear-gradient(225deg,transparent,transparent_10px,black_10px)]",
        className,
      )}
    >
